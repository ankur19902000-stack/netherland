...
const { getJobData, getJobSchema, TOTAL_JOBS, jobTitles, companies, netherlandsLocations, industries } = require('./jobData');
...
nav{background:#e67e22;color:#fff;padding:0 1.5rem;display:flex;align-items:center;justify-content:space-between;height:60px;position:sticky;top:0;z-index:100}
...
.hero{background:linear-gradient(135deg,#e67e22 0%,#d35400 50%,#a04000 100%);color:#fff;padding:3rem 1.5rem;text-align:center}
...
.search-bar input:focus,.search-bar select:focus{border-color:#e67e22}
...
.filter-chip.active,.filter-chip:hover{background:#e67e22;color:#fff;border-color:#e67e22}
...
.job-card:hover{border-color:#e67e22;transform:translateY(-2px)}
...
.btn-apply{padding:.55rem 1.3rem;background:#e67e22;color:#fff;border:none;border-radius:8px;font-weight:700;font-size:.85rem;cursor:pointer;transition:background .2s}
...
.btn-apply-big{padding:.85rem 2.5rem;background:#e67e22;color:#fff;border:none;border-radius:10px;font-weight:700;font-size:1rem;cursor:pointer;transition:background .2s}
.btn-apply-big:hover{background:#d35400}
...
.pagination .current{background:#e67e22;color:#fff;border-color:#e67e22}
.breadcrumb{font-size:.82rem;color:#888;margin-bottom:1rem}
.breadcrumb a{color:#e67e22}
.info-box{background:#fff;border-radius:12px;padding:1.25rem 1.5rem;border-left:4px solid #e67e22;margin-bottom:1rem;font-size:.88rem}
...
.modal input:focus{border-color:#e67e22}
.modal .btn-submit{width:100%;padding:.75rem;background:#e67e22;color:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:.95rem}
.modal .btn-submit:hover{background:#d35400}
...
${AD_TOP}
<nav>
  <a class="brand" href="/"><span>NL</span>JOBS<span>.nl</span></a>
  <div class="nav-links">
    <a href="/">Home</a>
...
${AD_BOTTOM}
<footer>
  &copy; 2025 NLJOBS.nl — <strong>100,000 Jobs</strong> in the Netherlands |
...
<div class="job-card">
  ...
  const websiteSchema = {
    ...
    "name": "NLJOBS.nl",
    "url": DOMAIN,
    "description": "Netherlands' largest job portal with 100,000 job listings — remote and on-site across all 12 provinces",
    "potentialAction": {
      "@type": "SearchAction",
  ...
  const body = `
<div class="hero">
  <h1>Find Your Dream Job in the <span class="accent">Netherlands</span></h1>
  <p>100,000 verified job listings — remote & on-site — across all 12 provinces</p>
  <form action="/jobs" method="get" style="display:flex;gap:.75rem;max-width:580px;margin:0 auto;flex-wrap:wrap">
    ...
    <select name="location" style="flex:1;min-width:140px;padding:.7rem;border-radius:8px;border:none;font-size:.85rem">
      ...
      <option value="noord-holland">North Holland</option>
      <option value="zuid-holland">South Holland</option>
      <option value="utrecht">Utrecht</option>
      <option value="noord-brabant">North Brabant</option>
    </select>
  ...
  <div class="stat-bar">
    ...
    <div class="stat"><strong>12</strong><span>Provinces</span></div>
...
<div class="container">
  <div class="info-box">
    🇳🇱 Netherlands' most comprehensive job board — browse <strong>50,000 remote jobs</strong> and <strong>50,000 on-site jobs</strong> across all industries.
  </div>
   ${AD_MIDDLE}
  ...
  res.send(renderHTML({
    title: 'NLJOBS.nl — 100,000 Jobs in the Netherlands | Remote & On-site',
    meta: 'Find your next job in the Netherlands. 100,000 verified listings — 50,000 remote and 50,000 on-site jobs across all 12 provinces.',
    bodyContent: body,
...
<div class="hero" style="padding:1.75rem 1.5rem">
  <h1 style="font-size:1.8rem">Browse <span class="accent">100,000 Jobs</span> in the Netherlands</h1>
...
<div class="container">
  ...
  res.send(renderHTML({
    title: `Netherlands Jobs — Page ${page} of ${totalPages.toLocaleString()} | NLJOBS.nl`,
    meta: `Browse ${TOTAL_JOBS.toLocaleString()} jobs in the Netherlands. Page ${page}. Remote and on-site positions across all industries.`,
    bodyContent: body,
    schema: null
  }));
});
...
app.get('/jobs/:id', (req, res) => {
  ...
  if (!id || id < 1 || id > TOTAL_JOBS) {
    return res.status(404).send(renderHTML({
      title: 'Job Not Found | NLJOBS.nl',
      meta: 'This job listing was not found.',
      bodyContent: `<div class="container" style="text-align:center;padding:4rem 1.5rem"><h1>404 — Job Not Found</h1><p style="margin:1rem 0 2rem">This job may have been filled or removed.</p><a href="/jobs" style="color:#e67e22">← Browse All Jobs</a></div>`,
      schema: null
    }));
  }
...
<div class="container">
  ...
  <div class="job-detail">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.5rem">
        ...
        <span style="font-size:.8rem;color:#888">Job ID: NL-${String(job.id).padStart(6, '0')}</span>
      </div>
    </div>
  ...
  <div style="text-align:center;margin-top:1.5rem">
    <a href="/jobs" style="color:#e67e22;font-weight:600">← Browse All 100,000 Jobs</a>
  </div>
</div>`;

  res.send(renderHTML({
    title: `${job.title} at ${job.company} — ${job.location} | NLJOBS.nl`,
    meta: `${job.title} job at ${job.company}. ${job.isRemote ? 'Remote' : job.location}. ${job.salary}. Apply now on NLJOBS.nl.`,
    bodyContent: body,
    schema
  }));
});
...
app.get('/sitemap', (req, res) => {
  const body = `
<div class="container">
  <h1 style="margin-bottom:1rem">Sitemap — NLJOBS.nl</h1>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;margin-top:1rem">
    <div class="job-card">
      ...
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/" style="color:#e67e22">🏠 Home</a>
        <a href="/jobs" style="color:#e67e22">📋 All Jobs (100,000)</a>
        <a href="/jobs?type=remote" style="color:#e67e22">🌐 Remote Jobs (50,000)</a>
        <a href="/jobs?type=onsite" style="color:#e67e22">🏢 On-site Jobs (50,000)</a>
      </div>
    </div>
    <div class="job-card">
      <div class="card-title">XML Sitemaps</div>
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/sitemap.xml" style="color:#e67e22">📄 Sitemap Index</a>
        <a href="/sitemap-1.xml" style="color:#e67e22">📄 Sitemap 1 (Jobs 1–1,000)</a>
        <a href="/sitemap-2.xml" style="color:#e67e22">📄 Sitemap 2 (Jobs 1,001–2,000)</a>
    ...
    <div class="job-card">
      ...
      <div style="display:flex;flex-direction:column;gap:.5rem;margin-top:.75rem;font-size:.88rem">
        <a href="/jobs/1" style="color:#e67e22">Job #1 (First Remote Job)</a>
        <a href="/jobs/50000" style="color:#e67e22">Job #50,000 (Last Remote Job)</a>
        <a href="/jobs/50001" style="color:#e67e22">Job #50,001 (First On-site Job)</a>
        <a href="/jobs/100000" style="color:#e67e22">Job #100,000 (Last On-site Job)</a>
      </div>
    </div>
  </div>
</div>`;
  ...
  res.send(renderHTML({
    title: 'Sitemap | NLJOBS.nl',
    meta: 'Complete sitemap of NLJOBS.nl with 100,000 job listings across the Netherlands.',
    bodyContent: body,
    schema: null
  }));
});
...
app.listen(PORT, () => {
  console.log(`🇳🇱 NLJOBS.nl running on ${DOMAIN}`);
  console.log(`📋 ${TOTAL_JOBS.toLocaleString()} job pages ready`);
  console.log(`🏢 ${companies.length} companies hiring in the Netherlands`);
  console.log(`📍 ${netherlandsLocations.length} locations across the Netherlands`);
});
